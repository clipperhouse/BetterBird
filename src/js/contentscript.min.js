...
L.title.appendNotifier();L.hide().content.hide();q.before(L);return L.show();};D=function(ab){var Z,ac,aa,ad;ad=decodeURIComponent(ab.query).replace("+"," ");
ac=ab.results.filter(O).filter(a(ad)).length;aa="search-query";Z=L.content.findByData("a",aa,ad);if(Z.length===0){Z=$("<a>").data(aa,ad).href("/#!/mentions/").text(ad).addNotifier().click(function(){$(this).clearNotifier();
...
}r.hide();Q.append(r);return r.show();});};J=function(ac,ad,ae){var ab,Z,aa;Z=$("<input type='checkbox'>").id("bb-option-"+ac).checked(w[ac]).change(function(){w[ac]=$(this).is(":checked");
